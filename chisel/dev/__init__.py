...
class NgramModelProbability(object):
    ...
    def learning_from_wc_df(self, wc_df):
        self._Xs = get_wc_df_vars(wc_df)
        self._ngram_wc_df = wc_df.set_index(self._Xs)["n"]
        self._ngram_jp_df = self._ngram_wc_df / self._ngram_wc_df.sum()
        
        self._n = len(self._Xs)
        
        
        self._ngram_wc_dfs[self._n] = self._ngram_wc_df
    ...
    def learning_from_texts(self, texts, with_complete_tail=True):
        self._texts = texts
        wc_df =  wc_df_add(*map(functools.partial(text2wordcountdf,n=self._n, with_complete_tail=with_complete_tail), self._texts))
...
class NgramModelProbabilityV2(object):
    def __init__(self, n=3, eps=1e-9):
        self._n = n
        self._eps = eps
    ...
    def learning_from_wc_df(self, wc_df):
        self._ngram_wc_df = wc_df
        self._Xs = get_wc_df_vars(self._ngram_wc_df.reset_index())
        
        self._ngram_jp_df = self._ngram_wc_df.set_index(self._Xs)["n"] / self._ngram_wc_df.set_index(self._Xs)["n"].sum()
        self._ngram_jp_df = self._ngram_jp_df.reset_index()
        self._ngram_jp_df.columns = self._Xs + ["p"]
    ...
    def learning_from_texts(self, texts, with_complete_tail=True):
        self._texts = texts
        self._ngram_wc_df = wc_df_add(*map(functools.partial(text2wordcountdf,n=self._n, with_complete_tail=with_complete_tail), self._texts))
        self._Xs = get_wc_df_vars(self._ngram_wc_df.reset_index())
    ...
    @property
    def ngram_jp_series(self):
        return self._ngram_jp_df.set_index(self._Xs)["p"]
    
    @property
    def ngram_wc_series(self):
        return self._ngram_wc_df.set_index(self._Xs)["n"]
    

    def select_ngram_jp_series(self, **Xs_conditions):
        proj_df = self._ngram_jp_df.copy()
        
        for X in Xs_conditions:
            proj_wc_df = proj_wc_df[proj_wc_df[X] == Xs_conditions[X]]
        return self._ngram_jp_df.set_index(self._Xs)["p"]
    

    def select_ngram_wc_series(self):
        return self._ngram_wc_df.set_index(self._Xs)["n"]
    
    
    
    
    def __call__(self, *Xs, **Xs_conditions):
        
        _Xs = [] 
        
        for X in Xs:
            assert X in self._Xs
        ...
        #print "Xs = ", Xs
        #print "Xs_conditions = ", Xs_conditions
        #print " _Xs = ", _Xs
        
        proj_wc_df = self._ngram_wc_df.groupby(_Xs).sum().reset_index().sort("n",ascending=False)
        
        for X in Xs_conditions:
            proj_wc_df = proj_wc_df[proj_wc_df[X] == Xs_conditions[X]]
        
        return type(self)().learning_from_wc_df(proj_wc_df)